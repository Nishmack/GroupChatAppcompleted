...
createGroup.addEventListener("click", () => {
  ...
  axios
    .get(`${baseURL}/home/group/create`, { headers: { Authentication: token } })
    .then((res) => {
      ...
      groupCreationPopupinner +=
        `<div class="overlay-button-maincontainer">` +
        `<div class="overlay-button-container">` +
        `<button>` +
        `<a id="closebutton" type="submit" href="#">Close` +
        `</a>` +
        `</button>` +
        `<button>` +
        `<a id="createbutton" type="submit" onclick="createfunction()" href="#">Create</a>` +
        `</button>` +
        `</div>` +
        `</div>`;
      document.getElementById("group-creation-popup").innerHTML =
        groupCreationPopupinner;
    });
});
...
function createfunction() {
  ...
  axios
    .post(`${baseURL}/home/group/create`, groupData, {
...
function getCommonGroupChats() {
  ...
  axios
    .get(`${baseURL}/home/chatusers`, { headers: { Authentication: token } })
    .then((res) => {
...
function getCommonGroupChatBody() {
  let chatBody =
    `<div class="chat-container" id="group-chat-box-main-container">` +
  ...
  let groupChatBox =
    `<input type="text" required class="chat-input-field" id="group-chat" >` +
...
function sendButton() {
  ...
  axios
    .post(`${baseURL}/home/chat`, chatData, {
      headers: { Authentication: token },
    })
    .then((res) => {
  ...
  axios
    .get(`${baseURL}/home/newchats?lastmessageid=${id}`, {
      headers: { Authentication: token },
    })
    .then((res) => {
...
function showOnscreenGroups(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    groupsNameInner +=
      `<li id="${data[i].groupId}" onclick = "getChatBody();startgroupchat(${data[i].groupId});startgroupchatContent(${data[i].groupId})">` +
      data[i].groupName +
      `</li>`;
  }
...
function openForm(groupId) {
  ...
  let groupChatBox = ``;
  groupChatBox +=
    `<input class="choose-file-input" type="file" accept="image/*" id="group-chat-image">` +
...
function startgroupchat(groupId) {
  ...
  groupChatBox +=
    ...
    `<button class="image-send-button" class="open-button" onclick="openForm(${groupId})">Image</button>` +
    `</input>`;
...
function startgroupchatContent(groupId) {
  ...
  const messages = axios
    .get(`${baseURL}/home/group/getmessage?groupId=${groupId}`, {
      headers: { Authentication: token },
    })
    .then((res) => {
      ...
      for (let i = 0; i < res.data.content.length; i++) {
        ...
        if (res.data.content[i].userName === userName) {
          if (
            res.data.content[i].chatcontent.includes(
            ...
            groupChatcontent +=
              ...
              `<div class="chat-content-field">` +
              `<div>` +
              res.data.content[i].chatcontent +
              `</div>` +
              `<div class="chat-time">` +
              timeOnly +
              `</div>` +
      ...
      document.getElementById("group-chat-message").innerHTML =
        groupChatcontent;
      let groupChatTitle = ``;
      groupChatTitle +=
        ...
        `</div>` +
        `<div>` +
        res.data.groupMembersLength +
        `Members` +
        `</div>` +
        `</div>` +
        `<div class="edit-button">` +
        `<button class="edit-groups">` +
        `<a class="edit-button-section" type="submit" id="edit" onclick ="editGroupContent(${groupId})" href="#popup2">Edit</a>` +
        `</button>` +
        `</div>`;
...
async function editGroupContent(groupId) {
  const token = localStorage.getItem("token");
  await axios
    .get(`${baseURL}/home/group/information?groupId=${groupId}`, {
      headers: { Authentication: token },
    })
    .then(async (response) => {
      const groupName = JSON.stringify(response.data.groupName);
      await axios
        .get(`${baseURL}/home/group/create`, {
          headers: { Authentication: token },
        })
        .then((res) => {
          ...
          groupEditPopupinner +=
            ...
            `<div class="overlay-button-container">` +
            `<button>` +
            `<a href="#" id="closebutton" type="submit" onclick="closeEditfunction()">Close</a>` +
            `</button>` +
            `<button>` +
            `<a href="#" id="createbutton" type="submit" onclick="updateFunction(${groupId},'${res.data.users.length}')">Update</button>` +
            `</button>` +
            `</div>` +
            `</div>`;

          document.getElementById("group-edit-popup").innerHTML =
            groupEditPopupinner;
        });
    });
}
...
function sendmessage(groupId) {
  ...
  if (document.getElementById("group-chat").value === "") {
    ...
    axios
      .post(
        `${baseURL}/home/group/sendmessage`,
        { groupId: groupId, content: groupChatContent },
        { headers: { Authentication: token } }
      )
      .then((res) => {
...
async function sendmessageAsImage(groupId) {
  ...
  await axios
    .post(`${baseURL}/home/group/sendimage`, formData, {
      headers: { Authentication: token },
    })
