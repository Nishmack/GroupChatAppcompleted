...
exports.getUsers = async (req, res) => {
  ...
  for (let i = 0; i < user.length; i++) {
    users.push({
      ...
      userId: jwt.sign(user[i].id, process.env.JWT_SECRET),
    });
  }
  res
    .status(200)
    .json({ success: true, message: "group created", users: users });
};
...
exports.createGroup = async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, message: "group created", groups: groupsIncluded });
};
...
async function myGroups(id) {
  ...
  for (let i = 0; i < groupsIncluded.length; i++) {
    const groupMembersCount = await GroupMember.count({
      where: { groupId: groupsIncluded[i].groupId },
    });
    let grp = await Groups.findOne({
      where: { id: groupsIncluded[i].groupId },
    });
    groupsIncludedName.push({
      groupName: grp.groupname,
      groupId: groupsIncluded[i].groupId,
    });
  }
...
exports.sendGroupMessage = async (req, res) => {
  let grpMember = await GroupMember.findOne({
    where: { userId: req.user.id, groupId: req.body.groupId },
  });
...
exports.getGroupMessage = async (req, res) => {
  ...
  res.json({
    ...
    groupMembersLength: grpMemberLength,
  });
};
