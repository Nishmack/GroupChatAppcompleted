...
function authentication(id, username, isAlive) {
  return jwt.sign(
    { id: id, userName: username, isAlive: isAlive },
    "f244c652502fdfa22f797cd8bea18894c943939899feb0f6b85cfba16d41e6419224d4894b9f622ae6a3ac2f3b7ef8cdf674f21ecb728c47f6276839f711244c"
  );
}
...
exports.login = async (req, res) => {
  try {
    const data = await User.findAll({
      where: { userphone: req.body.usernumber },
    });
    if (data.length === 0) {
          ...
          if (result == true) {
            res
              ...
              .json({
                ...
                token: authentication(data[0].id, data[0].username, true),
              });
          } else {
            res
              .status(401)
    ...
    res
      ...
      .status({
        success: false,
        message: "authentication is failed due to server error",
      });
  }
};
